...
description: Orchestrate ECL's Power Automate meeting-summary work. Use as the entry point for requests to plan, set up, build, validate, troubleshoot, or release the Teams transcript summarisation flow. First inventory the available tools, identities, permissions, and environment; then select and coordinate only the relevant implementation procedure.
---
